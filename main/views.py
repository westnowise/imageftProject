...
# def index2(request):
    # 비디오 스트림 열기
    cap = cv2.VideoCapture('C:/Users/parks/Desktop/Django/project1/static/img/Test_ft.mp4')
    fps = cap.get(cv2.CAP_PROP_FPS)  # 원래 동영상의 프레임 속도 사용

    # 웹캠 이미지 가져오기 (알파 채널을 포함하여 읽기)
    webcam_image = cv2.imread('C:/Users/parks/Desktop/Django/project1/static/img/removed_bg_image.png', cv2.IMREAD_UNCHANGED)
    ...
    # 동영상과 오디오 합성
    video = mp.VideoFileClip(os.path.join(output_folder, 'output.mp4'))
    audio = mp.AudioFileClip('C:/Users/parks/Desktop/Django/project1/static/img/Test_ft.mp4')
    final_clip = video.set_audio(audio)
    final_clip.write_videofile(os.path.join(output_folder, 'final_output.mp4'))
    ...
    return render(request, 'video/video.html', {'video_url': os.path.join('media', 'output', 'final_output.mp4')})
